...
/// Data model for PDF report generation.
struct PDFReportData {
    let user: User
    let startDate: Date
    let endDate: Date
    let entriesByDate: [Date: [FoodEntry]]
    let drinkEntries: [DrinkEntry]

    var totalCalories: Int {
        entriesByDate.values.flatMap { $0 }.compactMap(\.calories).reduce(0, +)
    }

    var totalProtein: Double {
        entriesByDate.values.flatMap { $0 }.compactMap(\.protein).reduce(0, +)
    }

    var totalCarbs: Double {
        entriesByDate.values.flatMap { $0 }.compactMap(\.carbs).reduce(0, +)
    }

    var totalFat: Double {
        entriesByDate.values.flatMap { $0 }.compactMap(\.fat).reduce(0, +)
    }

    var totalWaterIntake: Double {
        drinkEntries.reduce(0) { $0 + $1.amount }
    }
}
...
final class PDFGenerator {
    ...
    private func drawHeader(at y: CGFloat, data: PDFReportData, contentWidth: CGFloat) -> CGFloat {
        ...
        let userLine = "Prepared for: \(data.user.displayName)"
        userLine.draw(at: CGPoint(x: margin, y: currentY), withAttributes: detailAttributes)
        currentY += lineHeight

        let dateLine = "Period: \(dateFormatter.string(from: data.startDate)) - \(dateFormatter.string(from: data.endDate))"
    ...
    private func drawSummarySection(at y: CGFloat, data: PDFReportData, contentWidth: CGFloat) -> CGFloat {
        ...
        "Summary".draw(at: CGPoint(x: margin, y: currentY), withAttributes: sectionTitleAttributes)
        currentY += 25
        ...
        let summaryLines = [
            ...
            "Total Protein: \(String(format: "%.1f", data.totalProtein))g",
            "Total Carbohydrates: \(String(format: "%.1f", data.totalCarbs))g",
            "Total Fat: \(String(format: "%.1f", data.totalFat))g",
            "Total Water Intake: \(String(format: "%.0f", data.totalWaterIntake))ml"
        ]
    ...
    private func drawFooter(pageRect: CGRect) {
        ...
        let footerText = "Generated by NutriBalance • Balance your nutrition, transform your life"
